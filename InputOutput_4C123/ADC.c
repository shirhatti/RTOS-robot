...
// Provide a function that initializes Timer3A to trigger ADC
...
#define TIMER_CTL_TAOTE         0x00000020  // GPTM TimerA ADC Output Trigger
                                            // Enable
...
// promoted to an ADC0 controller interrupt.  Hardware Timer3A
...
//  delay = SYSCTL_RCGCTIMER_R;   // allow time to finish activating
//	
//  TIMER3_CTL_R &= ~(TIMER_CTL_TAEN|TIMER_CTL_TBEN);    // disable Timer3A during setup
//  TIMER3_CTL_R |= (TIMER_CTL_TAOTE|TIMER_CTL_TBOTE);   // enable Timer3A trigger to ADC
//  TIMER3_CFG_R = TIMER_CFG_16_BIT;             // configure for 16-bit timer mode
//  TIMER3_TAMR_R = TIMER_TAMR_TAMR_PERIOD;   // configure for periodic mode, default down-count settings
...
//  TIMER3_CTL_R |= (TIMER_CTL_TAEN|TIMER_CTL_TBEN);   // enable Timer3A 16-bit, periodic, no interrupts
//  ADC0_PC_R = 0x01;         // configure for 125K samples/sec
//  ADC0_SSPRI_R = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
//  ADC0_ACTSS_R &= ~0x0C;    // disable sample sequencer 3
//  ADC0_EMUX_R = (ADC0_EMUX_R&0xFFFF00FF)+0x5500; // timer trigger event
//  ADC0_SSMUX3_R = channelNum;
//	ADC0_SSMUX2_R = channelNum2;
//  ADC0_SSCTL3_R = 0x06;          // set flag and end
//	ADC0_SSCTL2_R = 0x06;
//  ADC0_IM_R |= 0x0C;             // enable SS3 interrupts
//  ADC0_ACTSS_R |= 0x0C;          // enable sample sequencer 3
//	
...
void ADC0_InitTimer3ATriggerSeq3PD3(uint32_t period){
  ...
  ADC0_SSPRI_R = 0x0123;        // 3) sequencer 0 is highest, sequencer 3 is lowest
  SYSCTL_RCGCTIMER_R |= SYSCTL_RCGCTIMER_R3;   // 4) activate timer3 
  delay = SYSCTL_RCGCGPIO_R;
  TIMER3_CTL_R &= ~TIMER_CTL_TAEN;    // disable timer3A during setup
  TIMER3_CTL_R |= TIMER_CTL_TAOTE;    // enable timer3A trigger to ADC
  TIMER3_CFG_R = TIMER_CFG_16_BIT;          // configure for 16-bit timer mode
  TIMER3_TAMR_R = TIMER_TAMR_TAMR_PERIOD;   // configure for periodic mode, default down-count settings
  TIMER3_TAPR_R = 0;            // prescale value for trigger
  TIMER3_TAILR_R = period-1;    // start value for trigger
  TIMER3_IMR_R = 0x00000000;    // disable all interrupts
  TIMER3_CTL_R |= TIMER_CTL_TAEN;   // enable Timer3A 16-b, periodic, no interrupts
...
void ADC0_InitTimer3BTriggerSeq2PD2(uint32_t period){
  ...
  ADC1_SSPRI_R = 0x0123;        // 3) sequencer 0 is highest, sequencer 3 is lowest
  SYSCTL_RCGCTIMER_R |= SYSCTL_RCGCTIMER_R3;   // 4) activate timer3 
  delay = SYSCTL_RCGCGPIO_R;
  TIMER3_CTL_R &= ~TIMER_CTL_TBEN;    // disable timer3A during setup
  TIMER3_CTL_R |= TIMER_CTL_TBOTE;    // enable timer3A trigger to ADC
  TIMER3_CFG_R = TIMER_CFG_16_BIT;          // configure for 16-bit timer mode
  TIMER3_TBMR_R = TIMER_TBMR_TBMR_PERIOD;   // configure for periodic mode, default down-count settings
  TIMER3_TBPR_R = 0;            // prescale value for trigger
  TIMER3_TBILR_R = period-1;    // start value for trigger
  TIMER3_IMR_R = 0x00000000;    // disable all interrupts
  TIMER3_CTL_R |= TIMER_CTL_TBEN;   // enable Timer3A 16-b, periodic, no interrupts
  ADC1_ACTSS_R &= ~0x08;        // 5) disable sample sequencer 3
  ADC1_EMUX_R = (ADC1_EMUX_R&0xFFFF0FFF)+0x5000; // 6) timer trigger event
